...
class DeviceListView(ttk.Frame):
    def __init__(
        ...
        filter_frame = ttk.LabelFrame(self, text=tr("Filters"))
        filter_frame.pack(fill="x", padx=8, pady=6)

        ttk.Label(filter_frame, text=tr("Search")).grid(row=0, column=0, sticky="w", padx=6, pady=6)
        ...
        self.tree.heading("asset_no", text=tr("Asset No"))
        self.tree.heading("device_type", text=tr("Type"))
        self.tree.heading("model", text=tr("Model"))
    ...
    def _build_form(self) -> None:
        form_frame = ttk.LabelFrame(self, text=tr("New Device Ticket"))
        ...
        self.state_var = tk.StringVar(value=tr("DeviceState.active"))

        ttk.Label(form_frame, text=tr("Asset No")).grid(row=0, column=0, sticky="w", padx=6, pady=4)
        ...
        ttk.Label(form_frame, text=tr("Type")).grid(row=1, column=0, sticky="w", padx=6, pady=4)
        ttk.Entry(form_frame, textvariable=self.device_type_var).grid(row=1, column=1, sticky="ew", padx=6, pady=4)

        ttk.Label(form_frame, text=tr("Model")).grid(row=1, column=2, sticky="w", padx=6, pady=4)
        ttk.Entry(form_frame, textvariable=self.model_var).grid(row=1, column=3, sticky="ew", padx=6, pady=4)

        ttk.Label(form_frame, text=tr("Version")).grid(row=2, column=0, sticky="w", padx=6, pady=4)
        ttk.Entry(form_frame, textvariable=self.version_var).grid(row=2, column=1, sticky="ew", padx=6, pady=4)

        ttk.Label(form_frame, text=tr("Status")).grid(row=2, column=2, sticky="w", padx=6, pady=4)
        ttk.Combobox(
            form_frame,
            textvariable=self.state_var,
            values=states_display("DeviceState", ["active", "standby", "maintenance", "retired"]),
            state="readonly",
        ).grid(row=2, column=3, sticky="ew", padx=6, pady=4)

        ttk.Label(form_frame, text=tr("Description")).grid(row=3, column=0, sticky="nw", padx=6, pady=4)
        ...
        ttk.Button(button_frame, text=tr("Create Device"), command=self._add_device).pack(side="right")
            ...
            if not keyword_lower or keyword_lower in label.lower():
                ...
                self.tree.insert(
                    ...
                    values=(
                        ...
                        device.model,
                        device.version,
                        state_display("DeviceState", device.state),
                    ),
                )
    ...
    def _add_device(self) -> None:
        ...
        if not asset_no:
            messagebox.showerror(tr("Error"), tr("Asset No is required"))
            return
        ...
        state = state_to_physical("DeviceState", self.state_var.get().strip(), "active")
        ...
        try:
            ...
            self.state_var.set(tr("DeviceState.active"))
